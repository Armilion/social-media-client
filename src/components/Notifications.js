...
class Notifications extends Component{
	...
	onMenuOpened = () => {
		let unreadNotificationsId = this.props.notifications.filter(notification => !notification.read).map(notification => notification.notificationId);
				...
				notifications.map(notification => {
					...
					return (
						<MenuItem key={notification.createdAt} onClick={this.handleClose}>
							{icon}
							<Typography color="default" variant="body1" component={Link} to={`/user/${notification.recipient}/scream/${notification.screamId}`}>
